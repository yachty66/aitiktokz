...
const SELECTORS = {
  ...
  // Prompt textarea (placeholder varies; select visible textarea)
  promptTextarea: 'textarea[aria-invalid="false"]:not([disabled])',
...
async function uploadImage(page: import('playwright').Page, imagePath: string): Promise<void> {
  ...
  // There are two columns: "Guide Hera" (left) and "Import" (right).
  // Prefer the right-hand "Import" input when both exist.
  // Prefer the Import column: its inputs have `multiple` attribute
  const dialog = page.locator('div[role="dialog"]').last();
  await dialog.waitFor({ state: 'visible', timeout: 30000 });
  const importInputs = dialog.locator('input[type="file"][multiple]');
  const importCount = await importInputs.count();
  if (importCount > 0) {
    await importInputs.first().setInputFiles(imagePath);
    return;
  }

  // Fallback: choose the right‑most visible input inside the dialog
  const inputs = dialog.locator(SELECTORS.uploadInput);
  await inputs.first().waitFor({ state: 'visible', timeout: 30000 });
  const count = await inputs.count();
  let target = inputs.first();
  if (count > 1) {
    const handles = await inputs.elementHandles();
    let rightIdx = 0;
    let maxX = -Infinity;
...
async function enterPrompt(page: import('playwright').Page, promptText: string): Promise<void> {
  // Prefer the first visible textarea in the prompt control
  let el = page.locator(SELECTORS.promptTextarea).first();
  try {
    await el.waitFor({ state: 'visible', timeout: 30000 });
  } catch {
    // Fallback to any visible textarea (not the hidden mirror)
